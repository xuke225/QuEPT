...
from quantize.quantizer import WeightQuantizer, ActQuantizer, KVQuantizer
...
def wrap_to_quant_model(model, args):
    '''
    ...
    for name, module in model.named_modules():
        ...
        if isinstance(module,torch.nn.Linear):
            quantlinear = int_linear_fake.QuantLinear(module,args)
            set_op_by_name(model, name, quantlinear)  
            del module  
        elif isinstance(module,(RMSN, LlamaRMSNorm)):
            quantnorm = QuantRMSNorm(module, 16, args)
            set_op_by_name(model, name, quantnorm)  
            del module 

def wrap_to_quant_vlm_model(model, args):
    '''
    replace nn.Linear and norm layer to correspond quantization counterparts
    '''
    for name, module in model.named_modules():
        ...
        if not args.quant_vision:
            if isinstance(module, torch.nn.Linear):
                if model.__class__.__name__ in ["InternVL2", "InternVLChatModel"]:
                    if not name.startswith('language_model.model.'):
                        continue
                elif model.__class__.__name__ in ["LlavaLlamaForCausalLM", "LlavaQwenForCausalLM", "InternLM2ForCausalLM"]:
            ...
            if isinstance(module, torch.nn.Linear):
                if 'mm_projector' in name:
                    continue
                if model.__class__.__name__ in ["InternVL2", "InternVLChatModel", "LlavaLlamaForCausalLM", 
                                                "LlavaQwenForCausalLM", "InternLM2ForCausalLM"
                                            ]:
                    quantlinear = int_linear_fake.QuantLinear(module, args)
                    set_op_by_name(model, name, quantlinear)
                    del module
...
def init_weight_quantizer(args, model):
    for name, module in model.named_modules():
        if isinstance(module,int_linear_fake.QuantLinear):
            ...
            module.weight_quantizer = WeightQuantizer(wbits, module.weight.shape, 
                                                      args.w_group_size, args.w_bit_list, 
                                                      True, dtype=module.weight.dtype)
            print(f'weight quantization: set {name} as w{args.w_bit_list}g{args.w_group_size} quantization')

def init_input_quantizer(args, model): 
    for name, module in model.named_modules():
        # skip lm_head quantization
        if isinstance(module,int_linear_fake.QuantLinear):
            ...
            module.use_act_quant = False
            quantized_shape = (1,module.in_features)
            module.input_quantizer = ActQuantizer(input_bits, args.input_bit_list)
            print(f'input activation quantization: set {name} as {args.input_bit_list}-bit quantization')
            ...
            module.use_act_quant = False
            module.output_quantizer =  ActQuantizer(input_bits, args.input_bit_list)
            print(f'output activation quantization: set {name} as {args.input_bit_list}-bit quantization')

def init_v_quantizer(args, model):
    ...
    for name, module in model.named_modules():
        if isinstance(module,int_linear_fake.QuantLinear) and 'v_proj' in name:
            ...
            module.output_quantizer = KVQuantizer(output_bits, quantized_shape,output_group_size)
            print(f'v-cache quantization: set {name} as {output_bits}-bit {output_group_size} groupsize quantization')
            
def init_k_quantizer(args, model):
    num_heads = model.config.num_attention_heads
    num_kv_heads = model.config.num_key_value_heads
    model_dim = model.config.hidden_size
    head_dim = model_dim // num_heads
    ...
    # for the quantization of k/v output (kv-cache quantization)
    for name, module in model.named_modules():
        if isinstance(module, rotation_utils.QKRotationWrapper): #!! k_value q_value rotate 
            ...
            module.k_quantizer = KVQuantizer(output_bits, quantized_shape,output_group_size) ##! for k_cache 
            print(f'k-cache quantization: set {name} as {output_bits}-bit {output_group_size} groupsize quantization')
...
def set_weight_lora_parameters(model, requires_grad):
    for n, m in model.named_parameters():
        if n.find("Bd")> -1 or n.find("Bu")> -1:
            m.requires_grad = requires_grad

def set_clip_parameters(model, requires_grad):
    for n, m in model.named_parameters():
        if n.find("bound")> -1:
            m.requires_grad = requires_grad

def weight_lora_parameters(model):
    params = []
    for n, m in model.named_parameters():
        if n.find("Bd")> -1 or n.find("Bu")> -1:
            params.append(m)
...
def trainable_parameters(model, args):
    params = []
    for n, m in model.named_parameters():
        if args.weight_lr > 0 and (n.find("Bd")> -1 or n.find("Bu")> -1): 
            params.append(m)
        if args.clip_lr > 0 and (n.find("bound")> -1):
            params.append(m)
    return iter(params)  

def trainable_parameters_num(model, args):
    params = []
    total = 0
    for n, m in model.named_parameters():
        if args.weight_lr > 0 and (n.find("Bd")> -1 or n.find("Bu")> -1): 
...
def set_grad_mask(model, done_bit):
    for m in model.modules():
        if isinstance(m, (QuantLinear)):
            m.update_grad_mask(done_bit)

def remove_grad_mask(model):
    for m in model.modules():
        if isinstance(m, (QuantLinear)):
            m.remove_grad_mask()
...
def model_bit_refactor(model, bit):
    for m in model.modules():
        if isinstance(m, (QuantLinear,QuantRMSNorm)):
            m.bit_refactor(bit)
            
def weight_act_bit_refactor(model, w_bit, a_bit):
    for m in model.modules():
        if isinstance(m, (QuantLinear,QuantRMSNorm)):
            m.weight_bit_refactor(w_bit)
            m.act_bit_refactor(a_bit)
...
def get_quant_config(args):
    ...
    quantization_config["w_bit_list"] = args.w_bit_list
    quantization_config["input_bits"] = args.input_bits
    quantization_config["rank_multiplier"] = args.rank_multiplier
    quantization_config["quant_type"] = args.quant_type
    # quantization_config["input_group_size"] = args.input_group_size
    quantization_config["input_bit_list"] = args.input_bit_list
    ...
    quantization_config["pre_rotate"] = args.pre_rotate
