...
def fuse_layer_norms(model):
    ...
    for layer in layers:
        ...
        elif model_type == model_utils.INTERNVL2_MODEL or model_type == model_utils.LLAVA_OV_MODEL:
            fuse_ln_linear(layer.post_attention_layernorm, [layer.mlp.up_proj, layer.mlp.gate_proj])
            fuse_ln_linear(layer.input_layernorm, [layer.self_attn.q_proj, layer.self_attn.k_proj, layer.self_attn.v_proj])
        else:
    ...
    if model_type == model_utils.INTERNVL2_MODEL:
...
def rotate_attention_output(layer, Q, model_type) -> None:
    ...
    if (model_type == model_utils.LLAMA_MODEL or 
        model_type == model_utils.QWEN2_MODEL or 
        model_type == model_utils.MISTRAL_MODEL or 
        model_type == model_utils.LLAVA_MODEL or 
        model_type == model_utils.LLAVA_OV_MODEL or 
        model_type == model_utils.INTERNVL2_MODEL
    ):
...
def rotate_model(model, rotate_mode='hadamard', online=False):
    model_type = model_utils.model_type_extractor(model)
    if model_type == model_utils.INTERNVL2_MODEL:
        Q = get_orthogonal_matrix(model.config.llm_config.hidden_size, rotate_mode,DEV)
    else:
    ...
    if model_type == model_utils.INTERNVL2_MODEL:
        ...
        model_dim = config.llm_config.hidden_size
    else:
        num_attention_heads = config.num_attention_heads
        num_key_value_heads = config.num_key_value_heads
        model_dim = config.hidden_size
    head_dim = model_dim // num_attention_heads
    Q_ov = get_orthogonal_matrix(head_dim,rotate_mode,DEV) #! R2 
    ...
    for idx, layer in enumerate(tqdm.tqdm(layers, unit="layer", desc="Rotating")):
        rotate_attention_inputs(layers[idx], Q, model_type) #! qkv linear R1 absorable
        rotate_attention_output(layers[idx], Q, model_type) #! o_proj linear R1 absorable
        rotate_mlp_input(layers[idx], Q, model_type)  #! up gate linear R1 absorable
        rotate_mlp_output(layers[idx], Q, model_type, online) #! down linear R1 absorable
        rotate_ov_proj(layers[idx], Q_ov, model_type, num_attention_heads,num_key_value_heads, head_dim) #! v o_porj R2 absorable
...
class QKRotationWrapper(torch.nn.Module):
    ...
    def __init__(self, func, config, online_had, *args, **kwargs):
        ...
        if hasattr(self.config, "llm_config"):
            ...
            self.model_dim = config.llm_config.hidden_size
