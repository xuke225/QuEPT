...
def get_wikitext2(tokenizer, train_size, val_size, seed, seqlen, test_only):
    print("get_wikitext2")
    try:
        # loacal_dataset = "/data/nlp_dataset/wikitext/wikitext-2-raw-v1"
        # set local path for faster loading
        print("load wikitext2 from local...")
        traindata = load_dataset("/home/lzc/workspace/datasets/wikitext-2-raw/wikitext.py", 'wikitext-2-raw-v1', split='train',trust_remote_code=True)
        testdata = load_dataset("/home/lzc/workspace/datasets/wikitext-2-raw/wikitext.py", 'wikitext-2-raw-v1', split='test',trust_remote_code=True)
    except:
        print("load wikitext2 from remote host...")
...
def get_c4(tokenizer, train_size, val_size, seed, seqlen, test_only):
    print("get_c4")
    C4_HOME = '/home/lzc/workspace/datasets/C4/'
    try:
        # set local path for faster loading
        traindata = load_dataset(
            C4_HOME, data_files={'train': 'en/c4-train.00000-of-01024.json.gz'}, split='train'
        )
        valdata = load_dataset(
            C4_HOME, data_files={'validation': 'en/c4-validation.00000-of-00008.json.gz'}, split='validation'
        )
    except:
        traindata = load_dataset(
...
def get_redpajama(tokenizer, train_size, val_size, seed, seqlen):
    print("get_redpajama")
    # try:
    loacal_dataset = "/home/lzc/workspace/datasets/RedPajama-Data-1T-Sample"
...
@torch.no_grad()
def test_ppl(args, model, tokenizer, datasets=['wikitext2']):
    results = {}
    for dataset in datasets:
        ...
        for i in tqdm(range(nsamples)):
            ...
            if "opt" in args.model_name.lower():
                outputs = model.model.decoder(batch) # opt
            elif "llama" in args.model_name.lower():
                outputs = model.model(batch)# llama
            hidden_states = outputs[0]
            logits = model.lm_head(hidden_states)
            shift_logits = logits[:, :-1, :]
            shift_labels = testenc[:, (i * seqlen) : ((i + 1) * seqlen)][:, 1:].to(model.lm_head.weight.device)
