...
class WeightQuantizer(nn.Module):
    def __init__(
        ...
        for gs in self.group_size:
            if gs != -1:
                # assert quantized_shape[-1] % gs == 0 
                # self.inc_groups.append(quantized_shape[-1] // gs)
    ...
    def forward(self, x:torch.Tensor):
        ...
        if gs != -1 and gs != x.shape[-1]:
            original_shape = x.shape
            if self.deficiency[bit_index] == 0:
                x = x.reshape(-1,gs)
            elif self.deficiency[bit_index] > 0:
                pad_zeros = torch.zeros((x.shape[0],self.deficiency[bit_index]),dtype=x.dtype,device=x.device)
        ...
        if self.learnable_clipping:
            if bit_index+1 <= len(self.upbound_gs):
                upbound_factor = self.sigmoid(self.upbound_gs[bit_index]) # clamp_ste(self.upbound_gs[bit_index],0,1.0)
                lowbound_factor = self.sigmoid(self.lowbound_gs[bit_index])# clamp_ste(self.lowbound_gs[bit_index],0,1.0)
            else:
                bound_index =  bit_index - (len(self.upbound_gs)+1)
                upbound_factor = self.sigmoid(self.upbound[bound_index]) # clamp_ste(self.upbound[bound_index],0,1.0) 
                lowbound_factor = self.sigmoid(self.lowbound[bound_index]) # clamp_ste(self.lowbound[bound_index],0,1.0) 
        ...
        scale = quant_range / (2**self.n_bits-1)
        scale = clamp_ste(scale, min=1e-5, max=1e4)
        round_zero_point = -(xmin/scale).clamp(min=-1e4, max=1e4).round()
        # round_zero_point = clamp_ste(round_ste(round_zero_point), qmin, qmax)
        x_int = round_ste(x / scale)
    ...
    def prepare_for_real_quant(self,x:torch.Tensor,bit):
        bit_index = self.bit_list.index(bit)
        gs = self.group_size[bit_index]
        ...
        qmin = 0
        qmax = 2 ** (bit) - 1
        scale = quant_range / (2**bit-1)
        scale = clamp_ste(scale, min=1e-5, max=1e4)
        round_zero_point = -(xmin/scale).clamp(min=-1e4, max=1e4).round()
        return scale, round_zero_point, gs
...
class KVQuantizer(nn.Module):
    ...
    def forward(self, x:torch.Tensor):
        ...
        scale_1 = 1 / scale
        zero = round_ste(-xmin * scale_1)
        scale = scale.repeat(1, 1, 1, self.group_size).reshape(init_shape)
        scale_1 = scale_1.repeat(1, 1, 1, self.group_size).reshape(init_shape)
        zero = zero.repeat(1, 1, 1, self.group_size).reshape(init_shape)
        x_int = clamp_ste(round_ste(x * scale_1) + zero, qmin, qmax)
