...
def preprocess_internlm_pure_pair(
        template_name,
        sources,
        tokenizer: transformers.PreTrainedTokenizer,
        num_image_token_list: list,
    ...
    for source in sources:
        ...
        for sentence in source:
            if sentence["from"].lower() == "human" and "<image>" in sentence["value"]:
    ...
    if not text_only:
        ...
        for conversation in conversations:
            for i in range(num_image):
                ...
                for sentence in source:
                    if sentence["from"].lower() == "human" and "<image>" in sentence["value"]:
                        sentence["value"] = sentence["value"].replace('<image>', image_tokens, 1)
            new_conversations.append(conversation)
        conversations = new_conversations
    ...
    for target, source in zip(targets, sources):
        prompts = [s["value"] for s in source]
        tokenized_lens = []
        ...
        for tokenized_len, speaker in zip(tokenized_lens, speakers):
            if speaker == "human":
                target[cur_idx: cur_idx + tokenized_len] = IGNORE_TOKEN_ID   
