...
from quantize.quant_funcs import pseudo_quantize_tensor
...
@torch.no_grad()
def auto_scale_block_vlm(module, module_kwargs, w_bit, q_config, input_feat, ans_mask, vis_mask, reweight_ratio_dict, loss_mode="mae"):
    ...
    def _search_module_scale(block, linears2scale: list, x, reweight_ratio=None, kwargs={}):
        ...
        for ratio in range(n_grid):
            ...
            if loss_mode == "mse":
                ...
                if ans_mask is not None and vis_mask is not None:
                    ans_mask_expand = ans_mask.unsqueeze(-1).expand_as(out) # 0
                    vis_mask_expand = vis_mask.unsqueeze(-1).expand_as(out).cuda() # 29360128
                    masked_diff_ans = ((org_out - out).float().abs() * ans_mask_expand) # 0
                    masked_diff_vis = ((org_out - out).float().abs() * vis_mask_expand) # 20000-50000
                    # reweight_ratio nan
                    # print(f"ans_mask_expand.sum():{ans_mask_expand.sum()} vis_mask_expand.sum():{vis_mask_expand.sum()} masked_diff_ans.sum():{masked_diff_ans.sum()}")
                    # print(f"masked_diff_vis.sum():{masked_diff_vis.sum()} reweight_ratio:{reweight_ratio}")
                    if reweight_ratio is not None:
            ...
            if is_best:
                ...
                best_scales = scales
            block.load_state_dict(org_sd)
        # print(f"best_ratio:{best_ratio} best_error:{best_error} best_scales:{best_scales}")
        if best_ratio == -1:
            print(history)
            raise Exception
        # print(best_ratio)
    ...
    elif "siglip_encoder" in str(module.__class__).lower() or "clip" in str(module.__class__).lower():
        # attention input
        scales_list.append(
            _auto_get_scale(
...
def apply_scale(module, scales_list, input_feat_dict=None):
    for prev_op_name, layer_names, scales in scales_list:
        ...
        elif isinstance(prev_op, nn.GELU) or isinstance(prev_op, BloomGelu, GELUActivation):
